...
// qty shortcut
$(function() {
	$('.qty-shortcut').on('click', function(event) {
		event.preventDefault();
		$('#quantity_wanted').val($(this).data('qty'));
	$("#quantity_wanted").trigger('change');
	
	});

	$unitProduct = $('.product-quantity.unit-product');
	if ($unitProduct.length > 0) {
		// $("#quantity_wanted").trigger("touchspin.updatesettings", {postfix:"x 10cm"});
		$postfix = $("#quantity_wanted").parent().find(".bootstrap-touchspin-postfix");
		$postfix.show();
		...
		$("#quantity_wanted").on('change', function(event) {
			...
			$postfix.text('x 10cm = ' + qty + 'cm');
			// $("#quantity_wanted").trigger("touchspin.updatesettings", {postfix:"x 10cm = " + qty + "cm"});
		});
		$("#quantity_wanted").trigger('change');
	}
});